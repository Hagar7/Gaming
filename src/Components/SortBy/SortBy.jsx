...
export default function SortBy() {
    ...
    let [SortByData,setSortByData] =useState([])
    let[previewData,setPreviewData]= useState(12)
   

   let SortBy = async()=>{
       let { data } = await axios.get(
           `https://free-to-play-games-database.p.rapidapi.com/api/games?sort-by=${params.sortby}`,
           {  
             headers: {
               "X-RapidAPI-Key":
   ...
   <>
    <div className="container">
        ...
        <>
           ...
           {SortByData.slice(0,previewData).map((item, index) => (
